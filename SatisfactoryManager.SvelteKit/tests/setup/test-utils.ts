...
import { users, games, userGames, modules, moduleVersions, items, buildings, recipes } from '../../src/lib/server/db/schema';
import { testUsers, testGames, testModules, testModuleVersions, testItems, testBuildings, testRecipes } from './fixtures';
...
export async function createTestEnvironment() {
	...
	const createdModuleVersions = await db.insert(moduleVersions).values(moduleVersionData).returning();
 ...
 */
export function createMockAuthenticatedRequest(method: string, url: string, body?: any, userId?: string) {
 ...
 */
export function createRequestWithParams(url: string, params: Record<string, string>, method = 'GET', body?: any) {
	return {
...
export async function createVersionedTestData() {
	...
	const versions = await db.insert(moduleVersions).values(testModuleVersions(module.id)).returning();
 ...
 */
export function createSearchRequest(baseUrl: string, query: string, filters: Record<string, string> = {}) {
	const url = new URL(baseUrl);
...
export async function createRelatedTestData() {
	const { users: testUsersData, games: testGamesData, modules: testModulesData, moduleVersions: testModuleVersionsData } = await createTestEnvironment();
