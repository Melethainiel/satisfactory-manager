...
export interface ItemState {
	...
	loadBuildingsForProduction: (gameId: string, itemId: string, productionType: 'extract' | 'craft' | 'power', recipeVersionId?: string) => Promise<Building[]>;
	clearResults: () => void;
...
class ItemStateClass implements ItemState {
	...
	async searchItems(gameId: string, query: string): Promise<ItemWithVersion[]> {
		...
		try {
			const res = await this.apiFetch(`/api/items?gameId=${encodeURIComponent(gameId)}&search=${encodeURIComponent(query.trim())}`);
	...
	async loadProductionOptions(gameId: string, itemId: string): Promise<ProductionOptions> {
		...
		try {
			const res = await this.apiFetch(`/api/items/${itemId}/production-options?gameId=${encodeURIComponent(gameId)}`);
	...
	async loadBuildingsForProduction(
		...
		try {
			...
			switch (productionType) {
				case 'extract':
					const extractorRes = await this.apiFetch(`/api/items/${itemId}/extractors?gameId=${encodeURIComponent(gameId)}`);
					if (!extractorRes.ok) throw new Error(`Failed to load extractors (${extractorRes.status})`);
					const extractorData = await extractorRes.json();
					buildings = extractorData.success ? extractorData.data : [];
					break;
					
				case 'power':
					const generatorRes = await this.apiFetch(`/api/items/${itemId}/generators?gameId=${encodeURIComponent(gameId)}`);
					if (!generatorRes.ok) throw new Error(`Failed to load generators (${generatorRes.status})`);
				...
				case 'craft':
					...
					const recipeRes = await this.apiFetch(`/api/recipes/versions/${recipeVersionId}/buildings`);
					if (!recipeRes.ok) throw new Error(`Failed to load recipe buildings (${recipeRes.status})`);
