...
export const ArchiveRecipeIngredientSchema = z.object({
	item: z.string().min(1).max(YAML_SECURITY_LIMITS.MAX_STRING_LENGTH),
	count: z.number().positive()
}).strict();

export const ArchiveRecipeProductSchema = z.object({
	item: z.string().min(1).max(YAML_SECURITY_LIMITS.MAX_STRING_LENGTH),
	count: z.number().positive()
}).strict();
...
export const ItemsArraySchema = z.array(ArchiveItemSchema).max(YAML_SECURITY_LIMITS.MAX_ARRAY_LENGTH);
export const BuildingsArraySchema = z.array(ArchiveBuildingSchema).max(YAML_SECURITY_LIMITS.MAX_ARRAY_LENGTH);
export const RecipesArraySchema = z.array(ArchiveRecipeSchema).max(YAML_SECURITY_LIMITS.MAX_ARRAY_LENGTH);
...
class YamlValidationService implements IYamlValidationService {
	...
	async validateAndParseItemsYamlStreaming(yamlContent: string): Promise<any[]> {
		try {
			...
			if (yamlContent.length < 1024 * 1024) { // < 1MB, use regular processing
