...
async function waitForDatabase() {
	const testDbUrl = process.env.TEST_DATABASE_URL || 'postgres://app:app@localhost:5433/satisfactory_test';
	...
	while (retries < MAX_RETRIES) {
		try {
			...
			if (retries === 1 || retries % 5 === 0) {
				console.log(`⏳ Waiting for database... (attempt ${retries}/${MAX_RETRIES}, ${timeLeft} retries left)`);
