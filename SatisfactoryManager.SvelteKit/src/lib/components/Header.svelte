...
<header class="navbar bg-base-100/80 backdrop-blur supports-[backdrop-filter]:bg-base-100/70 shadow-lg border-b border-base-200 px-4 sticky top-0 z-40 min-h-20 transition-colors">
		...
		{:else if authState.isAuthenticated && authState.user}
			<!-- User is logged in - show user info with dropdown -->
				<div class="user-menu-container">
					<button
						class="btn btn-ghost h-auto min-h-0 px-2 py-1 normal-case gap-2"
					...
					<ul
						id={popoverId}
						popover
						class="dropdown menu w-fit rounded-box bg-base-100 shadow-lg border p-2"
		...
		{#if authState.error}
			...
			<div class="toast toast-top toast-end">
				<div class="alert alert-error">
					...
					<button class="btn btn-sm btn-ghost" onclick={authState.clearError} aria-label="Close error">
						<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<style>
	...
	ul[popover] { animation: fadeScale 120ms ease-out; }
</style>
