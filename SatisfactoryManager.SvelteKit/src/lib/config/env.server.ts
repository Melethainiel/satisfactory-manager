...
const serverEnvSchema = z.object({
  ...
  AZURE_B2C_TENANT_ID: z.string()
    .min(1, 'AZURE_B2C_TENANT_ID must not be empty')
    .optional(),
  
  AZURE_B2C_POLICY: z.string()
    .min(1, 'AZURE_B2C_POLICY must not be empty')
    .optional(),
  
  AZURE_B2C_API_SCOPE: z.string()
    .min(1, 'AZURE_B2C_API_SCOPE must not be empty')
    .optional(),
...
export function getServerConfig(forceReload = false): ServerEnv {
  ...
  try {
    ...
    const envVars = {
      ...
      AZURE_B2C_KNOWN_AUTHORITIES: AZURE_B2C_KNOWN_AUTHORITIES || dynamicEnv.AZURE_B2C_KNOWN_AUTHORITIES,
